...
uniform bool NOD_THERMAL_DENOISE <
    ui_label = "Denoise Pass";
	ui_category = "Thermal";
> = true;

float4 Thermal_Denoise_KNN(float2 texcoord, float NoiseLevel, float LerpCoefficeint, float WeightThreshold, float CounterThreshold, float GaussianSigma) {
	float4 orig = tex2D(sNVG_Thermal, texcoord);

	if (!NOD_THERMAL_DENOISE)
		return orig;
	...
	for (int i = -WindowRadius; i <= WindowRadius; i++) {
		for (int j = -WindowRadius; j <= WindowRadius; j++) {
			texIJ = tex2D(sNVG_Thermal, texcoord + BUFFER_PIXEL_SIZE * float2(i, j)).rgb;
			weight = dot(orig.rgb - texIJ, orig.rgb - texIJ);
	...
	result = lerp(result, orig.rgb, lerpQ);
	
	return float4(result.r, result.g, result.b, orig.a);
}